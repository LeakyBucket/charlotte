...
defmodule Charlotte.ReqTest do
  ...
  describe "build_conn" do
    it "returns a conn record with the data from the request" do
      assert Charlotte.Req.build_conn([], [{:protocol, :tcp}]) == Charlotte.Req.Conn[req: [], verb: "GET", params: [cat: "meow", fish: "glug", dog: "woof"], req_headers: "", headers: [], path: "/bob", scheme: :http]
    end
  end

  describe "add_header" do
    it "adds a header when there aren't any" do
      assert Charlotte.Req.add_header(Charlotte.Req.Conn[], {"content-type", "text/html"}) == Charlotte.Req.Conn[headers: [{"content-type", "text/html"}]]
    end

    it "adds a header to an existing list" do
      conn = Charlotte.Req.Conn[headers: [{"content-type", "text/html"}]]

      assert Charlotte.Req.add_header(conn, {"location", "not-here"}) == Charlotte.Req.Conn[headers: [{"location", "not-here"}, {"content-type", "text/html"}]]
    end
  end
end
